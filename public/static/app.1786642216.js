...
async function getLiveBalance() {
  ...
  try {
    ...
    if (data.ok && typeof data.balance === 'number') {
      return { balance: data.balance, holdings: data.holdings || [] };
    }
    if (data.error) {
...
async function syncKisPositions() {
  try {
    ...
    if (kisHoldings.length === 0) {
      if (STATE.positions.length > 0) {
        addLog('info', `   ℹ️ KIS 보유종목 없음 — 기존 포지션 ${STATE.positions.length}개 초기화`);
        STATE.positions = [];
        savePositions();
      } else {
        addLog('info', `   ℹ️ KIS 보유종목 없음`);
      }
      renderPositions();
      updateStatsUI();
      return;
    ...
    for (const h of kisHoldings) {
      ...
      if (existing) {
        ...
        if (h.pnlPct) existing.pnlPct = h.pnlPct;
        newPositions.push(existing);
      } else {
        const avgP = h.avgPrice > 0 ? h.avgPrice : h.currentPrice;
        newPositions.push({
          ticker:       h.ticker,
        ...
        addLog('info', `   ➕ KIS 종목 추가: ${h.name || h.ticker} × ${h.qty}주 @ ${h.market === 'US' ? '$' : ''}${avgP}`);
      }
    }

    const added   = newPositions.filter(p => p.fromKis && !STATE.positions.find(e => e.ticker === p.ticker)).length;
    const removed = STATE.positions.filter(p => !newPositions.find(n => n.ticker === p.ticker)).length;
    STATE.positions = newPositions;
    savePositions();

    if (added > 0 || removed > 0) {
      addLog('info', `✅ KIS 포지션 동기화: +${added}추가 / -${removed}제거`);
    } else {
      addLog('info', `✅ KIS 포지션 동기화: ${newPositions.length}개 일치`);
    }
    renderPositions();
    updateStatsUI();
  } catch (e) {
...
function updateStatsUI() {
  ...
  if (STATE.mode === 'paper') {
    ...
    // ── 총자산 계산 ──
    // 통합증거금: 달러 잔고 필요 없음 — 원화(cash)로 미국주식 직접 매수
    // liveBalanceKrwForUs는 liveBalance와 동기화되므로 US 모드도 cash 그대로 사용
    const cashKrwForUs = STATE.liveBalanceKrwForUs > 0 ? STATE.liveBalanceKrwForUs : cash;
    let totalAsset = 0;
    if (mkt === 'KR')        totalAsset = cash + stockValKr;
    else if (mkt === 'US')   totalAsset = cashKrwForUs + stockValUsdKrw;
    else                     totalAsset = cash + stockValKr + stockValUsdKrw;
    ...
    if (mkt === 'US') {
      ...
      if (fetching && cash === 0 && !hasQueried) {
        ...
        cashEl.textContent = `${fmtPrice(cash)}원 (통합증거금)`;
      } else if (hasQueried) {
        cashEl.textContent = '0원 (잔고 없음 — ✏️ 입력 필요)';
      } else {
        cashEl.textContent = hasApiKey ? '조회 중…' : '미연결';
